...
var lhColors = [
  0xecba82, 0x4f3920, 0x8c7070, 0xf4eded, 0x4e6e5d, 0x3bc14a, 0x251351,
  0xc97b84, 0xa85751, 0x251351, 0xc7eae4, 0xa7e8bd, 0xffd972, 0xaba361,
  0x732c2c, 0x773344c,
];
...
function add_lighthouse(idx, p, q) {
  ...
  if (lighthouses[idx]) {
    ...
    if (trails) {
      ...
      trails.geometry.vertices[MAX_LINE_POINTS - 1] = new THREE.Vector3(
        p[0],
        p[1],
        p[2]
      );
  ...
  var geometry = new THREE.ConeGeometry(
    height / Math.cos((60 / 180) * Math.PI),
    height,
    4,
    1,
    true
  ...
  var lhBoxGeom = new THREE.CubeGeometry(0.075, 0.075, 0.075);

  lhBoxMaterial = new THREE.MeshBasicMaterial({
    color: lhColors[idx],
    flatShading: false,
    transparent: true,
    opacity: 0.75,
...
function survive_timecode_difference(most_recent, least_recent) {
  ...
  if (diff > 0xffffffff / 2) return 0xffffffff - diff;
  return diff;
}
  ...
  for (var key in angles) {
    ...
    for (var lh = 0; lh < 16; lh++) {
      ...
      if (angles[key][lh])
        for (var id in angles[key][lh]) {
          ...
          if (ang[0] === undefined || ang[1] === undefined) continue;

          for (var acode = 0; acode < 2; acode++) {
            if (
              survive_timecode_difference(when[key], ang[acode][1]) >
              visible_tolerance
            )
              continue;

            var augment = 0xf;
            if (lh) augment = augment << 8;
            if (acode) augment = augment << 4;
    ...
    for (var id in colors) {
      if (objs[key]) {
        ...
        if (material) material.color.setHex(colors[id]);
      }
    }
  }
}
    ...
    for (var lh = 0; lh < 16; lh++) {
      ...
      if (angles[key][lh])
        for (var id in angles[key][lh]) {
          ...
          const x_valid =
            ang[0] !== undefined &&
            survive_timecode_difference(when[key], ang[0][1]) <=
              visible_tolerance;
          const y_valid =
            ang[1] !== undefined &&
            survive_timecode_difference(when[key], ang[1][1]) <=
              visible_tolerance;
...
function create_tracked_object(info, external) {
  var sensorGeometry = new THREE.SphereGeometry(0.01, 32, 16);
  var group = new THREE.Group();
  var group_rot = new THREE.Group();
  group.sensors = [];
  var axesLength = 1;
  ...
  if (info.config && info.config.lighthouse_config) {
    ...
    if (trackref_from_head && trackref_from_imu) {
      var pa = [
        trackref_from_head[4],
        trackref_from_head[5],
        trackref_from_head[6],
      ];
      var qa = [
        trackref_from_head[0],
        trackref_from_head[1],
        trackref_from_head[2],
        trackref_from_head[3],
      ];
      ...
      var axes = new THREE.AxesHelper(0.075);
      imuref.add(axes);
    }
    ...
    for (var idx in info.config.lighthouse_config.modelPoints) {
      ...
      var normal = new THREE.Line(
        normalGeom,
        new THREE.LineBasicMaterial({ color: idx == 4 ? 0xff0000 : 0x00ff00 })
      );
...

  {
    var geometry = new THREE.SphereGeometry(1, 16, 16);
    var material = new THREE.MeshBasicMaterial({
      color: external ? 0x00ff00 : 0xff0000,
      opacity: 0.15,
      transparent: true,
      side: THREE.DoubleSide,
    });
  ...
  angVelocityGeom.vertices.push(
    new THREE.Vector3(0, 0, 0),
    new THREE.Vector3(0, 0, 0)
  );
  objs[info.tracker].angVelocity = new THREE.Line(
    angVelocityGeom,
    new THREE.LineDashedMaterial({ color: 0x00ffff, scale: 0.1 })
  );

  var accelGeom = new THREE.Geometry();
  accelGeom.vertices.push(
    new THREE.Vector3(0, 0, 0),
    new THREE.Vector3(0, 0, 0)
  );
  objs[info.tracker].accel = new THREE.Line(
    accelGeom,
...
function get_trails(obj, external) {
  if (trails[obj.tracker] == null) {
    ...
    var material = new THREE.LineBasicMaterial({
      color: trail_colors[trail_idx++ % trail_colors.length],
    });
    ...
    if (external) external_group.add(trails[obj.tracker]);
    else scene.add(trails[obj.tracker]);
  }
...
$(function () {
  $("#trails").change(update_trails);
});

function update_external() {
  var val = $("#external_displace")[0].checked ? 1.5 : 0;
  external_group.position.set(
    external2world[0],
    external2world[1] + val,
    external2world[2]
  );
  external_group.quaternion.set(
    external2world[4],
    external2world[5],
    external2world[6],
    external2world[3]
...
$(function () {
  $("#external_displace").change(update_external);
});

function update_show_model() {
  showModel = this.checked;
}
$(function () {
  $("#model").change(update_show_model);
});

function update_show_imu() {
  showIMU = this.checked;
}
$(function () {
  $("#imu").change(update_show_imu);
});
...
function update_fullcov(v) {
  ...
  const imageData = Uint8ClampedArray.from(
    fv
      .map((x) => [
        ...turbo(Math.min(1, Math.abs(fmax != 0 ? x / fmax : 0))),
        255,
      ])
      .flat()
  );
...
function display_matrix(name, rows, cols, fv) {
  ...
  const fmin = Math.min(...fv);
  fmax = Math.max(fmax, -fmin);
  const scaledData = fv.map((x) =>
    Math.min(1, Math.abs(fmax != 0 ? x / fmax : 0))
  );
  const imageData = Uint8ClampedArray.from(
    scaledData.map((x) => [...turbo(x), 255]).flat()
  );
  var canvas = covar_canvas[name];
...
function update_fullstate(v) {
  ...
  objs[obj.tracker].accel.geometry.vertices[1].set(
    obj.accel[0],
    obj.accel[1],
    obj.accel[2]
  );
...
function update_velocity(v) {
  ...
  objs[obj.tracker].velocity.geometry.vertices[1].set(
    obj.position[0],
    obj.position[1],
    obj.position[2]
  );
  ...
  objs[obj.tracker].angVelocity.geometry.vertices[1].set(
    obj.euler[0],
    obj.euler[1],
    obj.euler[2]
  );
...
function update_object(v, allow_unsetup, external) {
  ...
  if (objr) {
    ...
    if (objr.sensorref) objr.sensorref.visible = showModel;
...
function record_position(name, time, position) {
  ...
  $("#time")[0].max = max_time;
  if (position_history[name] === undefined) position_history[name] = [];
...
$(function () {
  ...
  $("#time").on("change", function (event, ui) {
    ...
    for (var i = 0; i < names.length; i++) {
      ...
      for (
        j = 0;
        j < position_history[name].length - 1 &&
        position_history[name][j].time < time;
        j++
      );
...
function get_rp_angles(tracker, lighthouse, sensor_id) {
  ...
  rp_angles[tracker][lighthouse][sensor_id] =
    rp_angles[tracker][lighthouse][sensor_id] || {};
...
var survive_log_handlers = {
  ...
  LH_UP: lhup,
  EXTERNAL_VELOCITY: function (v) {
    update_velocity(v, true, true);
  },
  EXTERNAL_POSE: function (v) {
    update_object(v, true, true);
  },
  DISCONNECT: function (v) {
    delete_tracked_object(v[1]);
  },
  ...
  B: function (v, tracker) {
    ...
    angles[obj.tracker][lighthouse][obj.sensor_id] =
      angles[obj.tracker][lighthouse][obj.sensor_id] || {};

    var axis = obj.plane;

    angles[obj.tracker][lighthouse][obj.sensor_id][axis] = [
      obj.angle,
      obj.timecode,
      -1,
    ];
  ...
  A: function (v, tracker) {
    ...
    angles[obj.tracker] = angles[obj.tracker] || {};
    angles[obj.tracker][obj.lighthouse] =
      angles[obj.tracker][obj.lighthouse] || {};
    angles[obj.tracker][obj.lighthouse][obj.sensor_id] =
      angles[obj.tracker][obj.lighthouse][obj.sensor_id] || {};

    angles[obj.tracker][obj.lighthouse][obj.sensor_id][obj.acode & 1] = [
      obj.angle,
      obj.timecode,
      obj.length,
    ];
  ...
  RA: function add_reproject_angle(v, tracker) {
    ...
    get_rp_angles(obj.tracker, obj.lighthouse, obj.sensor_id)[obj.axis] =
      obj.angle;
  },
  LOG: function (v) {
  ...
  I: function (v, tracker) {
    ...
    if (objs[obj.tracker]) {
      if (!downAxes[obj.tracker] && objs[obj.tracker]) {
        ...
        downAxes[obj.tracker].gyro_geom.vertices.push(
          new THREE.Vector3(0, 0, 0),
          new THREE.Vector3(0, 0, 0)
        );

        var line = new THREE.Line(
          downAxes[obj.tracker].gyro_geom,
      ...
      if (downAxes[obj.tracker].line) {
        downAxes[obj.tracker].gyro_geom.line.visible = downAxes[
          obj.tracker
        ].line.visible = showIMU;
      }
...
function add_survive_log_handler(name, entry) {
  survive_log_handlers[name] = entry;
}
...
function init() {
  ...
  fpv_camera = new THREE.PerspectiveCamera(VIEW_ANGLE * 2, ASPECT, 0.1, FAR);
  ...
  loader.load(
    "https://i.imgur.com/zUrlBcp.jpg", // 'https://i.imgur.com/7lmxb0b.jpg',
    onLoad,
    onProgress,
    onError
  );
...
function animate(time) {
  ...
  frames++;
  if (last_time != null) fps = fps * 0.5 + 1000 / (time - last_time + 1);
  // console.log(
  last_time = time;
}
